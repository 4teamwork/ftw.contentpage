...
** Teaser Image **
All ContentPages include EventPage and News are able to display a teaser image, which is stored
on the contentpage itself. It behaves like a regular block and shows also the description.
The teaser image related fields have their own write permission, check rolemap.xml.

** Portlet **
It's possible to enable an archive portlet for News and EventPages. In only works if the current container
has News/EventPages. The portlet is not assigned by default.
